...
//const {vtypes, ptypes, initContract } = pkg;

import {vtypes, ptypes, initContract } from './utils.js'
...
const near = await connect(Object.assign({ deps: { keyStore: new keyStores.UnencryptedFileSystemKeyStore('~/.ssh/near') } },nearConfig))
...
function create_seed(vtype, ptype, meta_url, rarity, edition){
	return account.functionCall(near.config.contractName, 'create_seed', {vtype: vtype, vsubtype: ptype, meta_url: meta_url, rarity:rarity, edition: edition}, 300000000000000, 0)
}


		await create_seed(vtypes.PLANT, ptypes.ORACLE, 
...
"https://arweave.net/33wa-6hW_vQAbkQ4a5ZXX7HGJMGR3M8ej-z9dvcnJ8k",
		...
		await create_seed(vtypes.PLANT, ptypes.PORTRAIT, 
...
"https://arweave.net/M7uwpTyRIZIohXBgIZUqoYDyxq1GyH3fkoT7CvN2iLE",
		...
		await create_seed(vtypes.PLANT, ptypes.MONEY, 
...
"https://arweave.net/dPBN2DGba13xI7IFqBoczspsHbTXUvmZ9sjKIuhU28o",
		...
		await create_seed(vtypes.HARVEST, ptypes.ORACLE, 
...
"https://arweave.net/eYJ3Ie8K3sVwvXSH5xtXtyi8PizJCzkXK4n7MCCMunE", // Oracle Voucher, by Oculardelusion
		...
		await create_seed(vtypes.HARVEST, ptypes.PORTRAIT, 
...
console.log(proms.length + " seeds added"); 
